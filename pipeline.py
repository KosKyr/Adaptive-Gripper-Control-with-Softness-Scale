...
class Pipeline():
    def __init__(self, label_color, box_color, mask_color, alpha, pad, confidence_threeshold, yolo_weights_path, dataset_path, use_data_set, use_rgbd=False, filter_classes=None, callback = None):
                        ...
                        if len(red_centers) == 2:
                            ...
                            if x1 < line_start[0] < x2 or x1 < line_end[0] < x2:
                                if width_px <= gripper_width:
                                    msg = "✅ Object can be gripped"
                                else:
                                    msg = "❌ Object too large to grip"
                                print(msg)
                                if self.callback:
                                    self.callback(msg)
